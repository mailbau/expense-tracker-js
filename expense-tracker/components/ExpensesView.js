...
export default function ExpensesView({ user }) {
    ...
    async function handleAdd() {
        ...
        const { error } = await supabase.from('expenses').insert({
            ...newItem,
            amount: parseFloat(newItem.amount), // Ensure it's a number
            user_id: user.id
        });
        ...
        <div className="space-y-6 max-w-5xl mx-auto pb-10">
            ...
            <div className="flex bg-slate-200 p-1 rounded-lg w-fit">
                <button
                    ...
                    className={`flex items-center gap-2 px-4 py-2 rounded-md font-medium transition ${activeTab === 'monthly' ? 'bg-white shadow text-slate-900' : 'text-slate-500 hover:text-slate-700'}`}
                >
                    <FaList /> Monthly Log
                </button>
                <button
                    onClick={() => setActiveTab('yearly')}
                    className={`flex items-center gap-2 px-4 py-2 rounded-md font-medium transition ${activeTab === 'yearly' ? 'bg-white shadow text-slate-900' : 'text-slate-500 hover:text-slate-700'}`}
                >
                    <FaChartBar /> Yearly Recap
                </button>
            </div>
                    ...
                    <div className="flex overflow-x-auto pb-2 gap-2 md:justify-center scrollbar-hide">
                        {Array.from({ length: 12 }, (_, i) => i + 1).map(m => (
                            <button
                                key={m}
                                onClick={() => setMonth(m)}
                                className={`px-4 py-2 rounded-full text-sm font-bold whitespace-nowrap transition ${month === m ? 'bg-slate-900 text-white' : 'bg-white border text-slate-500 hover:bg-slate-50'}`}
                            >
                        ...
                        <div className="pro-card p-6 lg:col-span-1 h-fit">
                            ...
                            <div className="space-y-3">
                                <div className="flex gap-2 p-1 bg-slate-100 rounded-lg">
                                    {['Expense', 'Income'].map(type => (
                                        <button
                                            ...
                                            className={`flex-1 py-1.5 text-sm rounded-md font-medium transition ${newItem.type === type ? 'bg-white shadow text-slate-900' : 'text-slate-500'}`}
                                        >
                                            {type}
                                        </button>
                                    ))}
                                ...
                                <input
                                    ...
                                    onChange={e => setNewItem({ ...newItem, amount: e.target.value })} // Keep as string for input
                                ...
                                <select value={newItem.category} onChange={e => setNewItem({ ...newItem, category: e.target.value })} className="w-full border rounded-lg p-2.5 text-sm">
                                    {newItem.type === 'Expense'
                                        ? ['Food', 'Transport', 'Bills', 'Shopping', 'Fun', 'Other'].map(c => <option key={c} value={c}>{c}</option>)
                                        : ['Salary', 'Bonus', 'Gift', 'Freelance', 'Other'].map(c => <option key={c} value={c}>{c}</option>)
                                    }
                                </select>
                                ...
                                {expenses.map(ex => (
                                    <div key={ex.id} className="group flex justify-between items-center p-3 hover:bg-slate-50 rounded-lg border border-transparent hover:border-slate-100 transition">
                                        <div className="flex items-center gap-3">
                                            <div className={`p-2 rounded-full ${ex.type === 'Income' ? 'bg-emerald-100 text-emerald-600' : 'bg-rose-100 text-rose-500'}`}>
                                                {ex.type === 'Income' ? <FaArrowUp size={12} /> : <FaArrowDown size={12} />}
                                            </div>
                                            <div>
                                                <p className="font-medium text-slate-800">{ex.item}</p>
                                                <p className="text-xs text-slate-400">{ex.expense_date} • {ex.category}</p>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-4">
                                            <span className={`font-mono font-medium ${ex.type === 'Income' ? 'text-emerald-600' : 'text-slate-900'}`}>
                                                {ex.type === 'Income' ? '+' : '-'} Rp {ex.amount.toLocaleString()}
                                            </span>
    ...
    return (
        <div className="pro-card p-5 flex items-center gap-4">
            ...
            <div>
                ...
                <p className={`text-2xl font-bold ${color}`}>Rp {value.toLocaleString()}</p>
            </div>
        </div>
    );
}